...
class ReminderRegistry {
  ...
  Future<void> _scheduleReminder(
    ...
    // A suppression marker set for today means the reminder was deliberately
    // silenced (e.g. the user already logged today). Re-arming must schedule
    // from tomorrow, never today — otherwise a restore or subscription-state
    // change would silently undo the suppression and re-nag. Only shift when
    // the computed time is STILL today: if the reminder time already passed,
    // `next` returned tomorrow and adding another day would skip a day.
